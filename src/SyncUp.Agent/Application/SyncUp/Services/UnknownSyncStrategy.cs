...
public class UnknownSyncStrategy : ISynchronizationStrategy
{
    ...
    public UnknownSyncStrategy(ISynchronizationQueue queue, IApiClient apiClient, IConfiguration config, ILogger<SyncUpService> logger)
    {
        _queue = queue;
    ...
    {
        List<FileEntry> _agentFilesList = new List<FileEntry>();
